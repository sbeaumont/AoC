...
def resolve(output):
    ...
    # Then it's an output key. Does that resolve to a number? If so, return it.
    command = outputs[output]
    if isinstance(command, int):
        return command

    # It's not a number, so we now have something to parse.
    command = command.split()

    # If it's just a single item it's either a number that's still a string or a single output key.
    # Otherwise it's a command to be parsed (AND, NOT...)
    if len(command) > 1:
        if len(command) > 2:
            # It's a binary operator!
            operator = command[1]

            # Using elif's instead of else's to make clear what operator is expected (and is more robust against broken data)
            if operator in ("AND", "OR"):
                first = resolve(command[0])
                second = resolve(command[2])
                if operator == 'AND':
        ...
        if command[0].isnumeric():
            outputs[output] = int(command[0])
        else:
            outputs[output] = resolve(command[0])
...
def load_data():
    ...
    for line in DATA.split('\n'):
        command = line.split('->')
        output = command[-1].strip()
        data[output] = command[0].strip()
    return data
