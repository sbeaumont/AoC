...
if not os.path.exists(f"AoC-2024-{next_puzzle_number}.py"):
    ...
    print(f"Copied AoC-2024-{next_puzzle_number}.py")
else:
...
with open(f"AoC-2024-{next_puzzle_number}-input.txt", 'w') as f:
    r = requests.get(f'https://adventofcode.com/2024/day/{next_puzzle_number}/input', cookies=cookie)
    f.writelines(r.text)

print("Wrote", f"AoC-2024-{next_puzzle_number}-input.txt")

with open(f"AoC-2024-{next_puzzle_number}-test-input.txt", 'w') as f:
    r = requests.get(f'https://adventofcode.com/2024/day/{next_puzzle_number}')
    test_input = re.search(r'<pre><code>(.*)<\/code><\/pre>', r.text, re.DOTALL).group(1)
    f.writelines(test_input)

print("Wrote", f"AoC-2024-{next_puzzle_number}-test-input.txt")
